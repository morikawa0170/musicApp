...
@section('content')
   <div class="container mt-5">
      <div class="card">
         ...
         <div class="card-body">
            ...
            <div class="row pt-2">
               @if ($state == "registered")
                  <form action="{{ route('playlists.update',$id) }}" id="update" method="POST">
                     @csrf
                     <input type="submit" value="更新" class="btn btn-primary ml-3 mr-2" onClick="return updateclick()">
                  </form>
                  <form action="{{ route('playlists.delete',$id) }}" method="POST">
                     @csrf
               ...
               @endif 
               <a href="https://open.spotify.com/playlist/{{ $playlistId }}" class="ml-3 d-block my-auto">編集</a>
            </div>
         </div>
      </div>
      <div>
   ...
   <script>
      ...
      var type="";
      function update() {
         var spotifyId ="{{ Auth::user()->spotifyId }}"; //現在ログインしているアカウントのspotifyid
